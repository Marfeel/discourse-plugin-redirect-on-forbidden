...
RSpec.describe "RedirectOnForbidden::RulesController" do
  ...
  describe "POST /rules" do
    ...
    it "creates a rule" do
      post base_path, params: { rule: { category_ids: [1, 2], url_pattern: "https://example.com/{category}/{slug}" } }
    ...
    it "returns errors for invalid rule" do
      post base_path, params: { rule: { category_ids: [], url_pattern: "http://bad.com" } }
  ...
  describe "PUT /rules/:id" do
    ...
    it "updates a rule" do
      ...
      put "#{base_path}/#{rule.id}", params: { rule: { url_pattern: "https://new.com/{slug}" } }
      expect(response.status).to eq(200)
